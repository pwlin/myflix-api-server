...
movies.search = function (req, res, db, cache) {
    ...
    if (utils.isEmptyObject(cachedItems)) {
        ...
        db.all('SELECT * FROM media WHERE mediatype = $mediatype AND (title LIKE $q OR originaltitle LIKE $q OR actor LIKE $q OR director LIKE $q OR year LIKE $q OR studio LIKE $q) ORDER BY title ASC;', {
            $mediatype: 'movies',
            $q: '%' + req.params.q + '%'
        }, function (err, rows) {
            rows = utils.mapMovieRows(rows);
