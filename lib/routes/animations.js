...
animations.search = function (req, res, db, cache) {
    ...
    if (utils.isEmptyObject(cachedItems)) {
        ...
        if ((req.params.q).match(/^s\:/ig)) {
            ...
            sqlQuery = 'SELECT * FROM media WHERE mediatype = $mediatype AND (title LIKE $q OR originaltitle LIKE $q OR actor LIKE $q OR director LIKE $q OR year LIKE $q OR studio LIKE $q) ORDER BY title ASC;';
        } else {
